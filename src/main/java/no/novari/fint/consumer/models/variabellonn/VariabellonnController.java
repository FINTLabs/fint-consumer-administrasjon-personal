package no.novari.fint.consumer.models.variabellonn;
...
public class VariabellonnController {
    ...
    @GetMapping
    public VariabellonnResources getVariabellonn(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getVariabellonnByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Variabellonn cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public VariabellonnResources getVariabellonnByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getVariabellonn(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private VariabellonnResources getVariabellonnByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_VARIABELLONN, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new VariabellonnResources();
    
        ArrayList<VariabellonnResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<VariabellonnResource>>() {});
    ...
    public VariabellonnResource getVariabellonnByKildesystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<VariabellonnResource> variabellonn = cacheService.getVariabellonnByKildesystemId(orgId, id);
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, VariabellonnResource.class);
    }
    ...
    public ResponseEntity putVariabellonnByKildesystemId(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody VariabellonnResource body
    ) {
        log.debug("putVariabellonnByKildesystemId {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, PersonalActions.UPDATE_VARIABELLONN, client);
        event.setQuery("kildesystemid/" + id);
