package no.novari.fint.consumer.models.personalressurs;
...
public class PersonalressursController {
    ...
    public PersonalressursResources getPersonalressurs(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getPersonalressursByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Personalressurs cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<PersonalressursResource> resources;
    ...
    @PostMapping("/$query")
    public PersonalressursResources getPersonalressursByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getPersonalressurs(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private PersonalressursResources getPersonalressursByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_PERSONALRESSURS, client);
        event.setOperation(Operation.READ);
        ...
        ArrayList<PersonalressursResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<PersonalressursResource>>() {});
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, PersonalressursResource.class);
    }
