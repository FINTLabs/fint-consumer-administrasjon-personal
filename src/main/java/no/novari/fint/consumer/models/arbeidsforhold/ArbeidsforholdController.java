package no.novari.fint.consumer.models.arbeidsforhold;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Arbeidsforhold", value = RestEndpoints.ARBEIDSFORHOLD, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class ArbeidsforholdController {
    ...
    public ArbeidsforholdResources getArbeidsforhold(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getArbeidsforholdByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Arbeidsforhold cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public ArbeidsforholdResources getArbeidsforholdByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getArbeidsforhold(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ArbeidsforholdResources getArbeidsforholdByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_ARBEIDSFORHOLD, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ArbeidsforholdResources();
    
        ArrayList<ArbeidsforholdResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ArbeidsforholdResource>>() {});
    ...
    public ArbeidsforholdResource getArbeidsforholdBySystemId(
        ...
        if (cacheService != null) {
            ...
            ArbeidsforholdResource arbeidsforhold = objectMapper.convertValue(response.getData().get(0), ArbeidsforholdResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(arbeidsforhold);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, ArbeidsforholdResource.class);
    }

    @PostMapping
    public ResponseEntity postArbeidsforhold(
