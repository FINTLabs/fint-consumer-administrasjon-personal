package no.novari.fint.consumer.models.fravar;
...
public class FravarController {
    ...
    public FravarResources getFravar(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFravarByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fravar cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public FravarResources getFravarByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFravar(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FravarResources getFravarByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_FRAVAR, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FravarResources();
    
        ArrayList<FravarResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FravarResource>>() {});
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, FravarResource.class);
    }
