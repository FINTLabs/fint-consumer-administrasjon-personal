package no.novari.fint.consumer.models.fastlonn;
...
@CrossOrigin
@RestController
@RequestMapping(name = "Fastlonn", value = RestEndpoints.FASTLONN, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class FastlonnController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Fastlonn cache is disabled.");
        }
    ...
    @GetMapping
    public FastlonnResources getFastlonn(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFastlonnByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fastlonn cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<FastlonnResource> resources;
    ...
    @PostMapping("/$query")
    public FastlonnResources getFastlonnByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFastlonn(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FastlonnResources getFastlonnByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_FASTLONN, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FastlonnResources();
    
        ArrayList<FastlonnResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FastlonnResource>>() {});
    ...
    public FastlonnResource getFastlonnByKildesystemId(
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(fastlonn);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, FastlonnResource.class);
    }

    @PostMapping
    public ResponseEntity postFastlonn(
