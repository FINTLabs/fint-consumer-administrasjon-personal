...
public class FasttilleggController {
    ...
    public FasttilleggResources getFasttillegg(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getFasttilleggByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Fasttillegg cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
    ...
    @PostMapping("/$query")
    public FasttilleggResources getFasttilleggByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getFasttillegg(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private FasttilleggResources getFasttilleggByOdataFilter(
        ...
        Event event = new Event(
                ...
                PersonalActions.GET_FASTTILLEGG, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new FasttilleggResources();
    
        ArrayList<FasttilleggResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<FasttilleggResource>>() {});
    ...
    public FasttilleggResource getFasttilleggByKildesystemId(
        ...
        if (cacheService != null) {
            ...
            return linker.mapAndResetLinks(fasttillegg);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        return statusCache.handleStatusRequest(id, orgId, linker, FasttilleggResource.class);
    }
